...
export default {
  ...
  joi(useStdTools) {
    return Joi.object({
      ...
      note: Joi.string().required().description("笔记，用作消息截断后保存的你的唯一记忆（将每次返回记录拼接汇总发送），需详细记录，参考范例设计。【重要】每次note均须总结上次note计划任务的执行情况。具体情况如何，看到什么，需关注什么，需修改什么，重要代码摘要、事件细节等，还有感悟！。不能简写,不许空"),
