...
var cleanJsonSchema, createStdToolTip, createToolTip, debug, debugWriteTimer, enableToolFold, formatToolsForPrompt, getCorePrompt, getJoiDoc, getMsgExample, getMsgProtocol, getNoteExample, getNoteExampleJson, getNoteExampleText, inferMissingToolIndex, lastConfigStr, tryExtractJSON, writeDebugConfig,
  indexOf = [].indexOf;
...
// 【兜底算法说明】
// 由于部分接口（如 Gemini 3.5 Flash 兼容层）在返回流式并行工具时会违规丢失 `index` 字段，
...
若被折叠，请用【工具缓存池展开工具】查看系统记忆中的完整内容。`.trim());
      ...
      if (ask.role === "assistant") {
        ...
        if (ask.extraContent && Object.keys(ask.extraContent).length > 0) {
          msg.extra_content = ask.extraContent;
        }
      }
          ...
          await (async(chunk) => {
            ...
            if (Array.isArray(chunk.choices) && chunk.choices.length > 0) {
              ...
              if (chunk.choices[0].delta.extra_content) {
                Object.assign(fullExtraContent, chunk.choices[0].delta.extra_content);
              }
              // 拼接工具调用
              if (deltaToolCalls) {
                for (index = j = 0, len = deltaToolCalls.length; j < len; index = ++j) {
                  ...
                  ((toolCallChunk) => {
                    var base, ref12, ref13, tmpIndex;
                    tmpIndex = toolCallChunk.index != null ? toolCallChunk.index : inferMissingToolIndex(toolCallChunk, fullToolCalls);
                    ...
                    if (toolCallChunk.extra_content) {
                      if ((base = fullToolCalls[tmpIndex]).extra_content == null) {
                        base.extra_content = {};
                      }
                      Object.assign(fullToolCalls[tmpIndex].extra_content, toolCallChunk.extra_content);
                    }
