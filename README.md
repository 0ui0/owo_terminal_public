...
### 编程功能
程序内置了完整的程序设计工作流，且所有的步骤皆可审批
和主流ai编程IDE不同的是，审批流程为：

1. 写入前审批，你必须依次审阅所有的文件，文件才会被真正写入

> 备注：你可以在【更多】菜单里将某些工具设置为白名单以便跳过审批

如果你在审批面板使用编辑器app打开，可以配置将审批结果的diff发送给ai，同时配置审批后是否用编辑器app打开该文件

如果你不使用编辑器app审批，而是直接在快捷面板内确认，将不会发送diff

2. 写入后复审+还原
开启了时光机备份后，每条对话都会创建一个还原点
你可以在对话处的【还原文件变动】，查看最新的文件变动相对于这个还原点发生了哪些修改
点击【查看】按钮，可以在编辑器内查看diff
你可以逐个审查这些变动
你可以勾选需要还原的文件

3. 直接使用时光机app
此外
你还可以在时光机app里查找到这个备份点，并预览这个备份点内的所有文件
你可以在时光机app里拖拽这个备份点的文件到资源管理器app覆盖文件，这个操作也能够触发还原
使用逻辑和mac的时间机器软件逻辑类似

---

### 快速打开app
使用快捷键ctrl+p或者cmd+p，可以快速打开工作目录内的文件
你可以使用全文搜索来快速定位文件

---

### 全局缩放
使用快键键ctrl/cmd + 加号/减号/零 可以缩放主界面UI

---

### 绘图器app
程序内置了相对全功能的svg编辑器，该编辑器的使用逻辑和flash软件的矢量绘制功能类似
它能够自动切割相交的线条和填充，并具备图层功能
你可以对他们编组或者双击进入组来进行快速编辑操作
ai可以调用这个绘图器app画画，你也可以用它来制作矢量图
